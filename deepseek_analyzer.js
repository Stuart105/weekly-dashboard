...
function callDeepSeek(prompt, callback) {
  // 从当前页面 URL 提取 EdgeOne 预览鉴权 token
  var qs = new URLSearchParams(window.location.search);
  var eoToken = qs.get('eo_token');
  var eoTime = qs.get('eo_time');
  var apiUrl = '/api/deepseek';
  if (eoToken && eoTime) {
    apiUrl += '?eo_token=' + encodeURIComponent(eoToken) + '&eo_time=' + encodeURIComponent(eoTime);
  }
  fetch(apiUrl, {
    method: 'POST',
