...
function renderAnalysis(el){{
  ...
  if(cats){{
    ...
    for(var i=0;i<cnames.length;i++){{
      ...
      p+=cn+'：流水¥'+(c.flow/10000).toFixed(1)+'万 | 占比'+(c.f_share?c.f_share.toFixed(1):'0')+'% | 同比'+(c.yoy>0?'+':'')+(c.yoy?c.yoy.toFixed(1):'0')+'% | 环比'+(c.mom>0?'+':'')+(c.mom?c.mom.toFixed(1):'0')+'% | 折扣'+(c.disc?c.disc.toFixed(1):'0')+'%\\n';
      if(c.group==='product'){{
        p+='  SKU在售：'+c.sku_s+' | 库存：'+(c.s_qty?c.s_qty:'0')+' | 动销率：'+(c.sku_u?c.sku_u.toFixed(1):'0')+'% | 匹配分析：'+(c.match_lbl||'')+'\\n';
      }}
    }}
  }}
  // Daily data
  ...
  for(var i=0;i<7&&i<D.daily.length;i++){{
    var d=D.daily[i];
    p+=days[i]+'：流水¥'+(d.f/10000).toFixed(1)+'万 | 目标¥'+(d.t/10000).toFixed(1)+'万 | 达成'+d.a.toFixed(1)+'% | 同比'+(d.y>0?'+':'')+d.y.toFixed(1)+'% | 客流'+d.v+'人 | 客单价¥'+d.tk.toFixed(0)+' | 连带率'+d.at.toFixed(2)+'\\n';
  }}
  // Seasonal data
  p+='\\n【新品季节数据】\\n';
  var seas=D.seas;
  if(seas){{
    for(var sk in seas){{
      var s=seas[sk];
      p+=sk+'：流水¥'+(s.f/10000).toFixed(1)+'万 | 折扣'+s.d.toFixed(1)+'% | SKU'+s.sku+'个 | 库存'+s.stock_qty+'件 | 动销率'+s.su.toFixed(1)+'% | 可满足率'+s.sat.toFixed(1)+'%\\n';
    }}
  }}
  // Mid-category data
  p+='\\n【中类汇总】\\n';
  ...
  // Summary insights
  p+='\\n【衍生指标】\\n';
  var worstDay='周六',bestDay='周日';
  if(D.daily&&D.daily.length>5){{ worstDay=D.daily[5].n;bestDay=D.daily[6].n; }}
  // Find worst/best
  var wi=0,bi=0;
  for(var i=0;i<7&&i<D.daily.length;i++){{ if(D.daily[i].a<D.daily[wi].a)wi=i;if(D.daily[i].a>D.daily[bi].a)bi=i; }}
  p+='达成最低日：'+D.daily[wi].n+'（'+D.daily[wi].a.toFixed(1)+'%）\\n';
  p+='达成最高日：'+D.daily[bi].n+'（'+D.daily[bi].a.toFixed(1)+'%）\\n';
  p+='客流+成交率双降：'+(D.flow_yoy>0&&D.conv_yoy<0?'是（客流↑但转化↓，进店不买问题突出）':'否')+'\\n';
  p+='折扣螺旋：'+(D.disc>40&&D.mom<-10?'是（高折扣+流水下降，打折失效信号）':'否')+'\\n';
  p+='\\n========================================\\n';
  ...
  p+='请用专业客观的语言，直接分析数据，不要评价数据本身的好坏（如"表现良好""有待提升"这类空洞评价）。每条分析控制在100-150字。';
